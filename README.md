...
A libp2p private network was created with four nodes. Each [node](https://github.com/gcp-development/ipfs-private-network/tree/main/node) will have the Identify("/ipfs/id/1.0.0") and Kademlia("/ipfs/lan/kad/1.0.0") behaviour configure. A node will be added to the libp2p private network and it will have to discover every other node in the network just by connecting to node-d.
